...
private const val ALPHA_MIN_VALUE = 0.01f
private const val ALPHA_MAX_VALUE = 0.99f
private val ALPHA_RANGE = ALPHA_MIN_VALUE..ALPHA_MAX_VALUE
...
internal fun Modifier.placeholder(
    ...
    if (animationSpec != null && (visible || placeholderAlpha >= ALPHA_MIN_VALUE)) {
    ...
    remember(color, shape, highlight) {
        drawWithContent {
            ...
            if (contentAlpha in ALPHA_RANGE) {
            ...
            } else if (contentAlpha >= ALPHA_MAX_VALUE) {
                // If the content alpha is > 99%, draw it with no alpha
                drawContent()
            }

            if (placeholderAlpha in ALPHA_RANGE) {
            ...
            } else if (placeholderAlpha >= ALPHA_MAX_VALUE) {
